...
let write_c program  number_steps path = 
  ...
  code := !code ^ ("void printBoolArray( long long value,  long long cpt_0123456789, const char *ident) {\n\
  printf(\"%s = \",ident);
  ...
  code := !code ^ (" long long binary_to_int(const char *bin_str) {\n\
...
code := !code ^ "\n long long read_rom(const char *rom,  long long i,  long long j) {
  ...
  let get_string_declaration x = 
    ...
    |TBit -> " long long "^(Hashtbl.find name x)^";\n long long "^(Hashtbl.find name x)^"_reg_version = 0;\n"
    |TBitArray n -> " long long "^(Hashtbl.find name x)^";\n" ^ " long long "^(Hashtbl.find name x)^"_reg_version = 0;\n" in
  ...
  let rec add_action_write_ram l =
    ...
    |(addr_size,word_size,we,write_addr,write_data,name)::q -> 
              (*le nom du tableau est donne quand on trouve l'instruction, rajouter les déclarations*)
                ...
                decl_ram := !decl_ram^" long long "^name^"[1ULL<<"^(string_of_int addr_size)^"];\n for ( long long iiiii=0; iiiii < (1ULL << ("^(string_of_int addr_size)^"));iiiii++){\n"^name^"[iiiii] = 0;};\n";
